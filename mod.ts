...
const workflowsPush = new Command()
  ...
  .option('-w, --create-triggers', 'Create webhook triggers for each workflow', {
    default: false,
  })
  .option(
    '--regenerate-secrets',
...
const workflowsList = new Command()
  ...
  .example(
    'Filter by tag',
    'quickflo workflows list --tags stripe -o abcd',
  )
  .example(
    'Multiple tags (OR)',
...
const packagesPublish = new Command()
  ...
  .option('--dry-run', 'Print the resolved publish plan without making API calls', {
    default: false,
  })
  .option('-j, --json', 'After the human-readable summary, emit the publish response as JSON', {
    default: false,
  })
  .example(
  ...
  .action(async (opts, packageRef) => {
    const tags = (opts.tags ?? []).flatMap((s: string) => s.split(',')).map((s) => s.trim()).filter(
      (s: string) => s.length > 0,
    );
...
const packages = new Command()
  .description('Package management commands (export/import composable solution bundles).')
